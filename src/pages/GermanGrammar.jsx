...
export default function GermanGrammar() {
  ...
  const tabs = [{ key: 'grammar', label: 'Grammar Overview' }, { key: 'vocabulary', label: 'Vocabulary Practice' }]
  return (
...
function PracticeSession() {
  ...
  const [list, setList] = useState([]), [origCount, setOrigCount] = useState(0)
  const [idx, setIdx] = useState(0), [show, setShow] = useState(false)
  const [known, setKnown] = useState(new Set())
  useEffect(() => { const data = localStorage.getItem(knownWordsKey); data && setKnown(new Set(JSON.parse(data))) }, [])
  const shuffle = arr => { for (let i = arr.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [arr[i], arr[j]] = [arr[j], arr[i]] } return arr }
  const load = (cat, knownOverride) => {
    const k = knownOverride !== undefined ? knownOverride : known
    const raw = categoriesMap[cat] || []
    const filtered = raw.filter(item => !k.has(item.german))
    const shuffled = shuffle([...filtered])
    setList(shuffled); setOrigCount(shuffled.length); setIdx(0); setShow(false)
  }
  useEffect(() => { load(category) }, [category])
  useEffect(() => { triggerResize() }, [list, idx, show])
  const save = s => localStorage.setItem(knownWordsKey, JSON.stringify([...s]))
  const resetKnown = () => { localStorage.removeItem(knownWordsKey); const newKnown = new Set(); setKnown(newKnown); load(category, newKnown) }
  return (
    <motion.div variants={fadeUpVariant} className="mb-12 w-full">
      ...
      <div className="flex flex-wrap justify-center gap-3 mb-4">
        {Object.keys(categoriesMap).map(c => (
          <button key={c} onClick={() => setCategory(c)} className={`px-4 py-2 rounded-full font-medium text-sm sm:text-base transition ${c === category ? 'bg-accent text-white shadow-md' : 'bg-[#1b2735] text-gray-200 hover:bg-accent hover:text-white'}`}>{c}</button>
        ))}
      </div>
      <div className="flex flex-wrap justify-center gap-4 mb-4">
        ...
        <button onClick={resetKnown} className="px-4 py-2 rounded-full font-semibold bg-red-600 text-white hover:bg-red-700 transition">Reset Known Words</button>
      </div>
      <div className="bg-[#1b2735] p-4 sm:p-6 rounded-xl shadow-md min-h-[200px] transition hover:shadow-lg flex flex-col items-center">
        {list[idx] ? <>
          ...
          <motion.div key={list[idx].english} initial={{ opacity: 0 }} animate={{ opacity: show ? 1 : 0 }} transition={{ duration: 0.5, ease: 'easeOut' }} className="mb-4 text-base bg-gray-800 p-4 rounded w-full">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <button onClick={() => { setShow(!show); triggerResize() }} className="px-4 py-2 bg-accent text-white rounded-full hover:bg-accent-dark transition">{show ? 'Hide Details' : 'Show Details'}</button>
            <button onClick={() => { setShow(false); if (idx < list.length - 1) setIdx(idx + 1) }} className="px-4 py-2 bg-accent text-white rounded-full hover:bg-accent-dark transition">Next Word</button>
            <button onClick={() => {
              if (!list[idx]) return
              const upd = new Set(known)
              upd.add(list[idx].german)
              setKnown(upd); save(upd)
              const newList = [...list]
      ...
      {origCount > 0 && <div className="mt-4 text-center text-xs sm:text-sm text-gray-400 w-full">
        <p>Progress: {origCount - list.length} / {origCount}</p>
        <progress className="w-full h-2 bg-gray-800 rounded" value={origCount - list.length} max={origCount} />
      </div>}
    </motion.div>
  )
}