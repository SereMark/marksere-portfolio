    ...
    <div className="font-sans min-h-screen bg-primary text-gray-100">
      <header className="bg-[#1b2735] shadow-md">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="text-2xl font-bold cursor-pointer transition-transform duration-300 hover:scale-105" onClick={() => handleNavClick('home')}>
          ...
          <nav className="hidden md:flex space-x-4 items-center">
            {navItems.map((item) => (
              <button
                ...
                className={`px-3 py-2 rounded transition hover:text-accent ${isActive(item.value) ? 'bg-accent text-[#1b2735] font-semibold' : ''}`}
              >
                {item.label}
              </button>
            ))}
          ...
          <div className="md:hidden">
            <button onClick={() => setNavOpen(!navOpen)} className="focus:outline-none text-white" aria-label="Toggle Mobile Menu">
            ...
            <nav className="flex flex-col p-2">
              {navItems.map((item) => (
                <button
                  key={item.value}
                  onClick={() => handleNavClick(item.value)}
                  className={`w-full text-left px-3 py-2 rounded my-1 transition hover:text-accent ${isActive(item.value) ? 'bg-gray-800 font-semibold' : ''}`}
                >
                  {item.label}
                </button>
              ))}
